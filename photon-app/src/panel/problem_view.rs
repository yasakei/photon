...
fn file_view(
    ...
    let icon = match severity {
        DiagnosticSeverity::ERROR => PhotonIcons::ERROR,
        _ => PhotonIcons::WARNING,
    };
    let icon_color = move || {
        let config = config.get();
        match severity {
            DiagnosticSeverity::ERROR => config.color(PhotonColor::PHOTON_ERROR),
            _ => config.color(PhotonColor::PHOTON_WARN),
        }
    };
...
fn related_view(
    ...
    stack((
        ...
        stack((
            svg(move || config.get().ui_svg(PhotonIcons::LINK)).style(move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
