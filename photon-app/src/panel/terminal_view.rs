...
use photon_rpc::terminal::TermId;

use super::kind::PanelKind;
use crate::{
    app::clickable_icon,
    command::{InternalCommand, PhotonWorkbenchCommand},
...
fn terminal_tab_header(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    stack((
        ...
        container(clickable_icon(
            || PhotonIcons::ADD,
            move || {
                workbench_command.send(PhotonWorkbenchCommand::NewTerminalTab);
            },
            || false,
            || false,
            || "New Terminal",
