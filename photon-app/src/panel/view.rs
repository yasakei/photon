    ...
    stack((
        h_stack((
            clickable_icon_base(
                move || {
                    if open.get() {
                        PhotonIcons::PANEL_FOLD_DOWN
                    } else {
                        PhotonIcons::PANEL_FOLD_UP
                    }
                },
                None::<Box<dyn Fn()>>,
                || false,
...
pub fn panel_container_view(
    ...
    let resize_drag_view = {
        ...
        move |position: PanelContainerPosition| {
            // Snapshot drag start in window coords plus the initial size.
            // `pointer_event.pos` is local to the handle (which itself moves
            // as the panel resizes), so a local delta stalls near zero. Using
            // window coords with the initial size avoids both the stall and the
            // double-counting/overshoot from re-reading the live size each move.
            ...
            let drag_state: RwSignal<Option<(Point, f64)>> = create_rw_signal(None);
            // Floem runs a full style + layout pass after every pointer event,
            // so with a high-report-rate mouse a drag would queue hundreds of
            // whole-window relayouts per second and the UI freezes. Apply size
            // changes at most once per frame; the latest target is stashed and
            // flushed on release so the panel always ends under the cursor.
            const APPLY_INTERVAL: Duration = Duration::from_millis(15);
            let last_apply: Rc<Cell<Instant>> =
                Rc::new(Cell::new(Instant::now() - 3 * APPLY_INTERVAL));
            let pending: Rc<Cell<Option<f64>>> = Rc::new(Cell::new(None));
            let apply_size: Rc<dyn Fn(f64)> = Rc::new(move |new_size_raw: f64| {
                let available_size = available_size.get_untracked();
                match position {
                    PanelContainerPosition::Left => {
                        let new_size = new_size_raw
                            .max(150.0)
                    ...
                    PanelContainerPosition::Bottom => {
                        ...
                        if (maximized
                            && new_size_raw < available_size.height - 50.0)
                            || (!maximized
                                && new_size_raw > available_size.height - 50.0)
                        {
            ...
            let apply_size_up = apply_size.clone();
            let pending_up = pending.clone();
            let last_apply_down = last_apply.clone();
            let pending_down = pending.clone();
            resize_handle()
                .on_down(move |window| {
                    let initial = panel_size.with_untracked(|s| match position {
                        PanelContainerPosition::Left => s.left,
                        PanelContainerPosition::Bottom => s.bottom,
                        PanelContainerPosition::Right => s.right,
                    });
                    last_apply_down.set(Instant::now() - 3 * APPLY_INTERVAL);
                    pending_down.set(None);
                    drag_state.set(Some((window, initial)));
                })
                .on_move(move |window| {
                    if let Some((start_window, initial)) = drag_state.get_untracked()
                    {
                        let delta = window - start_window;
                        let new_size_raw = match position {
                            PanelContainerPosition::Left => initial + delta.x,
                            PanelContainerPosition::Bottom => initial - delta.y,
                            PanelContainerPosition::Right => initial - delta.x,
                        };
                        let now = Instant::now();
                        if now.duration_since(last_apply.get()) >= APPLY_INTERVAL {
                            last_apply.set(now);
                            pending.set(None);
                            apply_size(new_size_raw);
                        } else {
                            pending.set(Some(new_size_raw));
                        }
                    }
                })
                .on_up(move || {
                    if let Some(new_size_raw) = pending_up.get() {
                        pending_up.set(None);
                        apply_size_up(new_size_raw);
                    }
                    drag_state.set(None);
                })
                .style(move |s| {
                    ...
                    // The strip sits fully inside the panel: anything sticking
                    // out of the panel is covered by a later sibling (the
                    // editor area), which hit-tests first and swallows the
                    // press, leaving only a couple of grabbable pixels.
                    let panel_size = panel_size.with(|s| match position {
                        PanelContainerPosition::Left => s.left,
                        PanelContainerPosition::Bottom => s.bottom,
                        PanelContainerPosition::Right => s.right,
                    });
                    s.absolute()
                        .apply_if(position == PanelContainerPosition::Bottom, |s| {
                            s.width_pct(100.0).height(6.0)
                        })
                        .apply_if(position == PanelContainerPosition::Left, |s| {
                            s.width(8.0)
                                .margin_left(panel_size as f32 - 8.0)
                                .height_pct(100.0)
                        })
                        .apply_if(position == PanelContainerPosition::Right, |s| {
                            s.width(8.0).height_pct(100.0)
                        })
                        .apply_if(is_dragging, |s| {
                            s.background(config.color(PhotonColor::EDITOR_CARET))
                                .apply_if(
    ...
    .style(move |s| {
        ...
        s.apply_if(!panel.is_container_shown(&position, true), |s| s.hide())
            ...
            .color(config.color(PhotonColor::PANEL_FOREGROUND))
    })
...
fn panel_picker(
    ...
    .style(move |s| {
        s.border_color(config.get().color(PhotonColor::PHOTON_BORDER))
            .apply_if(
                panels.with(|p| {
