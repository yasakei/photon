...
use photon_core::buffer::rope_text::RopeText;
use photon_rpc::source_control::FileDiff;
...
use crate::{
    ...
    config::{color::PhotonColor, icon::PhotonIcons},
                        ...
                        s.margin_top(10.0)
                            ...
                            .active(|s| {
                                s.background(config.color(
                                    PhotonColor::PANEL_HOVERED_ACTIVE_BACKGROUND,
                                ))
                            })
                            .selectable(false)
                    })
...
fn file_diffs_view(source_control: SourceControlData) -> impl View {
    ...
    let photon_command = source_control.common.photon_command;
    ...
    let view_fn = move |(path, (diff, checked)): (PathBuf, (FileDiff, bool))| {
        ...
        stack((
            ...
            label(move || folder.clone()).style(move |s| {
                s.text_ellipsis()
                    .flex_grow(1.0f32)
                    .flex_basis(0.0)
                    .color(config.get().color(PhotonColor::EDITOR_DIM))
                    .min_width(0.0)
                    .selectable(false)
            }),
            container({
                svg(move || {
                    let svg = match &diff {
                        FileDiff::Modified(_) => PhotonIcons::SCM_DIFF_MODIFIED,
                        FileDiff::Added(_) => PhotonIcons::SCM_DIFF_ADDED,
                        FileDiff::Deleted(_) => PhotonIcons::SCM_DIFF_REMOVED,
                        FileDiff::Renamed(_, _) => PhotonIcons::SCM_DIFF_RENAMED,
                    };
                ...
                .style(move |s| {
                    ...
                    let color = match &diff_for_style {
                        FileDiff::Modified(_) => PhotonColor::SOURCE_CONTROL_MODIFIED,
                        FileDiff::Added(_) => PhotonColor::SOURCE_CONTROL_ADDED,
                        FileDiff::Deleted(_) => PhotonColor::SOURCE_CONTROL_REMOVED,
                        FileDiff::Renamed(_, _) => {
                            PhotonColor::SOURCE_CONTROL_MODIFIED
                        }
                    };
            ...
            let discard = move || {
                photon_command.send(PhotonCommand {
                    kind: CommandKind::Workbench(
                        PhotonWorkbenchCommand::SourceControlDiscardTargetFileChanges,
                    ),
        ...
        .style(move |s| {
            ...
            s.padding_left(10.0)
                ...
                .hover(|s| {
                    s.background(config.color(PhotonColor::PANEL_HOVERED_BACKGROUND))
                })
        })
    };
