...
use crate::{
    ...
    workspace::{PhotonWorkspace, WorkspaceInfo},
};
...
pub enum SaveEvent {
    App(AppInfo),
    Workspace(PhotonWorkspace, WorkspaceInfo),
    RecentWorkspace(PhotonWorkspace),
    Doc(DocInfo),
    DisabledVolts(Vec<VoltID>),
    WorkspaceDisabledVolts(Arc<PhotonWorkspace>, Vec<VoltID>),
...
impl PhotonDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<PhotonWorkspace>> {
        let workspaces =
            std::fs::read_to_string(self.folder.join(RECENT_WORKSPACES))?;
        let workspaces: Vec<PhotonWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &PhotonWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: PhotonWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &PhotonWorkspace) -> String {
