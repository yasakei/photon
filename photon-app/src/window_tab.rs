...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, PhotonCommand,
        PhotonWorkbenchCommand, WindowCommand,
    },
    ...
    workspace::{PhotonWorkspace, PhotonWorkspaceType, TerminalTabState, WorkspaceInfo},
};
...
pub struct CommonData {
    pub workspace: Arc<PhotonWorkspace>,
    pub scope: Scope,
    ...
    pub photon_command: Listener<PhotonCommand>,
    pub workbench_command: Listener<PhotonWorkbenchCommand>,
    ...
    pub config: ReadSignal<Arc<PhotonConfig>>,
    pub proxy_status: RwSignal<Option<ProxyStatus>>,
    pub mouse_hover_timer: RwSignal<TimerToken>,
    pub breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, PhotonBreakpoint>>>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<PhotonConfig>>,
    /// Editor-only zoom delta in px, relative to the settings font size.
    /// Per window tab; persisted per workspace, never written to TOML.
    pub editor_zoom_delta: RwSignal<i32>,
    /// Settings-file editor font size the zoom delta applies on top of.
    /// Refreshed on config reload so ZoomReset tracks settings edits.
    pub editor_font_size_base: RwSignal<usize>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let photon_command = Listener::new_empty(cx);
                    ...
                    .map(|(path, breakpoints)| {
                        (
                            ...
                            breakpoints
                                .into_iter()
                                .map(|b| (b.line, b))
                                .collect::<BTreeMap<usize, PhotonBreakpoint>>(),
                        )
                    })
                    .collect(),
            );
        ...
        let window_tab_data = Self {
            ...
            common,
            editor_zoom_delta: cx.create_rw_signal(0),
            editor_font_size_base: cx.create_rw_signal(0),
        };

        {
            // Restore the persisted editor zoom (per workspace) on top of
            // the settings-file font size.
            let base = window_tab_data
                .common
                .config
                .get_untracked()
                .editor
                .font_size();
            window_tab_data.editor_font_size_base.set(base);
            let delta = workspace_info
                .as_ref()
                .map(|i| i.editor_zoom_delta)
                .unwrap_or(0);
            window_tab_data.editor_zoom_delta.set(delta);
            window_tab_data.apply_editor_zoom();

            // Restore saved terminal tabs (plain shells with their launch
            // workdirs). Empty means "keep the default single terminal".
            if let Some(info) = workspace_info.as_ref() {
                if !info.terminal_tabs.is_empty() {
                    window_tab_data.terminal.restore_terminal_tabs(
                        &info.terminal_tabs,
                        info.active_terminal_tab,
                    );
                }
            }
        }
        ...
        {
            // Built-in Discord rich presence: report the active file and
            // workspace whenever they change. Runs on the main thread; the
            // worker thread only receives plain strings.
            //
            // The `active_editor` memo only re-fires when `active_editor_tab`
            // changes (switching between tab groups). Switching files within the
            // same tab mutates `EditorTabData.active` through the inner
            // `RwSignal<EditorTabData>`, so we subscribe to that signal chain
            // directly to catch intra-tab switches.
            let main_split = window_tab_data.main_split.clone();
            let workspace = window_tab_data.workspace.clone();
            let config = window_tab_data.common.config;
            let tx = crate::discord::presence_sender();
            cx.create_effect(move |_| {
                // Reactive: re-fires when tabs are added/removed.
                let all_tabs = main_split.editor_tabs.get();
                // Reactive: re-fires when switching between tab groups.
                let active_tab_id = main_split.active_editor_tab.get();

                // (details text, open file path for the icon key, cursor line:col)
                let (details, file_path, cursor_loc) = active_tab_id
                    .and_then(|tab_id| all_tabs.get(&tab_id).copied())
                    .map(|tab_signal| {
                        // Subscribe to the inner RwSignal<EditorTabData> so
                        // we also re-fire when EditorTabData.active changes
                        // within the same tab.
                        tab_signal.with(|editor_tab| {
                            match editor_tab.children.get(editor_tab.active) {
                                Some((_, _, EditorTabChild::Editor(editor_id))) => {
                                    match main_split.editors.editor(*editor_id) {
                                        Some(editor) => {
                                            // Track the cursor so presence updates
                                            // live as the caret moves around.
                                            let offset = editor.cursor().get().offset();
                                            let (line, col) =
                                                editor.doc().buffer.with(|buffer| {
                                                    buffer.offset_to_line_col(offset)
                                                });
                                            let cursor_loc =
                                                Some(format!("Ln {}, Col {}", line + 1, col + 1));
                                            match editor.doc().content.get_untracked() {
                                                ...
                                                crate::doc::DocContent::Scratch { name, .. } => (
                                                    format!("Editing {name}"),
                                                    None,
                                                    cursor_loc,
                                                ),
                                                _ => (
                                                    "Editing untitled file".to_string(),
                                                    None,
                                                    cursor_loc,
                                                ),
                                            }
                                        }
                                        None => ("No file open".to_string(), None, None),
                                    }
                                }
                                Some((_, _, EditorTabChild::DiffEditor(_))) => {
                                    ("Editing diff".to_string(), None, None)
                                }
                                _ => ("No file open".to_string(), None, None),
                            }
                        })
                    })
                    .unwrap_or_else(|| ("No file open".to_string(), None, None));
                let small_image_key = file_path
                    .as_deref()
                    .and_then(crate::discord::asset_key_for_path)
                    .map(str::to_string);
                let state = match (workspace.path.as_ref().and_then(|p| {
                    p.file_name().map(|n| n.to_string_lossy().into_owned())
                }), cursor_loc) {
                    (Some(name), Some(loc)) => format!("{loc} · In {name}"),
                    (Some(name), None) => format!("In {name}"),
                    (None, Some(loc)) => loc,
                    (None, None) => "Browsing files".to_string(),
                };
                let enabled = config.get().core.enable_discord_presence;
                let _ = tx.send(crate::discord::PresenceUpdate {
                    details,
                    state,
                    small_image_key,
                    enabled,
...

        {
            ...
            window_tab_data.common.photon_command.listen(move |cmd| {
                window_tab_data.run_photon_command(cmd);
            });
        }
    ...
    /// Apply `editor_font_size_base + editor_zoom_delta` to this tab's
    /// local config clone. Editor views read `common.config`, so only
    /// editor text (code, gutter, lenses, completion) scales — the rest
    /// of the UI is untouched. Nothing is written to the settings file.
    pub fn apply_editor_zoom(&self) {
        let base = self.editor_font_size_base.get_untracked();
        if base == 0 {
            return;
        }
        ...
        if !change_plugins.is_empty() || builtin_lsp_changed {
            ...
            if config.core.auto_reload_plugin {
                let mut plugin_metas: HashMap<
                    String,
                    photon_rpc::plugin::VoltMetadata,
                > = self
                    .plugin
                    .installed
                    .get_untracked()
    ...
    pub fn run_photon_command(&self, cmd: PhotonCommand) {
        match cmd.kind {
    ...
    pub fn run_workbench_command(
        ...
        match cmd {
            ...
            // ==== UI ====
            // Editor-only zoom: adjusts this tab's font size without
            // touching the global UI scale. Persists per workspace via
            // the db (saved with the rest of the session state).
            ZoomIn => {
                self.editor_zoom_delta.update(|delta| {
                    *delta = (*delta + 1).min(16);
                });
                self.apply_editor_zoom();
            }
            ZoomOut => {
                self.editor_zoom_delta.update(|delta| {
                    *delta = (*delta - 1).max(-16);
                });
                self.apply_editor_zoom();
            }
            ZoomReset => {
                self.editor_zoom_delta.set(0);
                self.apply_editor_zoom();
            }
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/photon" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/photon" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
            ...
            | QuickJump8
            |             QuickJump9 => {
                // Invoked from palette/command (no Shift info), default to upward
                // (matches Alt+digit). Keyboard path (Alt+digit / Alt+Shift+digit)
                // is handled in `EditorData::run_command` where Shift selects
                // the direction (Alt = up, Alt+Shift = down).
                if let Some(digit) = cmd.quick_jump_digit() {
                    if let Some(editor) =
                        self.main_split.active_editor.get_untracked()
                    {
                        editor.quick_jump_to_line_end(digit, true);
                    }
                }
            }
    ...
    pub fn workspace_info(&self) -> WorkspaceInfo {
        ...
        WorkspaceInfo {
            ...
            breakpoints: self
                ...
                .collect(),
            editor_zoom_delta: self.editor_zoom_delta.get_untracked(),
            terminal_tabs: self.terminal_tabs_state(),
            active_terminal_tab: self
                .terminal
                .tab_info
                .with_untracked(|info| info.active),
        }
    }

    /// Snapshot plain shell terminals per tab for session restore.
    /// Run/debug terminals are skipped (tied to a live debug session).
    fn terminal_tabs_state(&self) -> Vec<TerminalTabState> {
        self.terminal
            .tab_info
            .with_untracked(|info| {
                info.tabs
                    .iter()
                    .filter_map(|(_, tab)| {
                        let terminals = tab.terminals.get_untracked();
                        let active_term_id = terminals
                            .get(tab.active.get_untracked())
                            .map(|(_, t)| t.term_id);
                        let mut workdirs = Vec::new();
                        let mut active = 0;
                        for (_, t) in terminals.iter() {
                            if t.run_debug.with_untracked(|r| r.is_some()) {
                                continue;
                            }
                            if Some(t.term_id) == active_term_id {
                                active = workdirs.len();
                            }
                            workdirs.push(t.workdir.get_untracked());
                        }
                        if workdirs.is_empty() {
                            None
                        } else {
                            Some(TerminalTabState { active, workdirs })
                        }
                    })
                    .collect()
            })
