...
use photon_rpc::{buffer::BufferId, plugin::PluginId, proxy::ProxyResponse};
...
use crate::{
    command::{CommandKind, InternalCommand, PhotonCommand, PhotonWorkbenchCommand},
...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let photon_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_multi_selection_command(
        ...
        // This is currently special-cased in Photon because floem editor does not have 'find'
        match cmd {
    ...
    fn run_move_command(
        &self,
        movement: &photon_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
    ...
    /// Alt quick-jump: move `digit` lines up (if `upward`) or down to the end
    /// of the target line. Digit `0` stays on the current line.
    /// Gutter shows `0` for the current line and `1`-`9` in the jump direction
    /// while Alt is held (above for Alt, below for Alt+Shift).
    pub fn quick_jump_to_line_end(
        &self,
        digit: usize,
        upward: bool,
    ) -> CommandExecuted {
        ...
        if !self
            .common
            .config
            .with_untracked(|c| c.editor.enable_alt_quick_jump)
        {
            return CommandExecuted::No;
        }
        if self
        ...
        let target_line = if digit == 0 {
            current_line
        } else if upward {
            current_line.saturating_sub(digit)
        } else {
            (current_line + digit).min(last_line)
        };

        // `LinePosition::Line` is 1-indexed, so add 1. Then go to end of line
        // so the caret lands at the end, as requested. Use empty modifiers so
        // Shift (used to pick the upward direction) doesn't extend selection.
        let no_mods = Modifiers::empty();
        self.run_move_command(
            &photon_core::movement::Movement::Line(
                photon_core::movement::LinePosition::Line(target_line + 1),
            ),
            None,
            no_mods,
        );
        self.run_move_command(
            &photon_core::movement::Movement::EndOfLine,
            None,
            no_mods,
        );
        CommandExecuted::Yes
            ...
            FocusCommand::JumpToNextSnippetPlaceholder => {
                self.snippet.update(|snippet| {
                    if let Some(snippet_mut) = snippet.as_mut() {
                        ...
                        {
                            let mut selection =
                                photon_core::selection::Selection::new();
                            let region = photon_core::selection::SelRegion::new(
                                *start, *end, None,
                            );
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &photon_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &photon_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    pub fn jump_highlight(&self, direction: InlineFindDirection) {
        ...
        if let Some(new_offset) = new_offset {
            self.run_move_command(
                &photon_core::movement::Movement::Offset(new_offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn apply_completion_item(&self, item: &CompletionItem) -> anyhow::Result<()> {
        ...
        let additional_edit: Vec<_> = item
            ...
            .flatten()
            .map(|edit| {
                let selection = photon_core::selection::Selection::region(
                    buffer.offset_of_position(&edit.range.start),
                    buffer.offset_of_position(&edit.range.end),
                );
                (selection, edit.new_text.as_str())
            })
            .collect::<Vec<(photon_core::selection::Selection, &str)>>();
    ...
    pub fn completion_apply_snippet(
        ...
        let mut selection = photon_core::selection::Selection::new();
        let (_tab, (start, end)) = &snippet_tabs[0];
        let region = photon_core::selection::SelRegion::new(*start, *end, None);
        selection.add_region(region);
        cursor.set_insert(selection);
    ...
    fn search_whole_word_forward(&self, mods: Modifiers) {
        ...
        if let Some((start, _end)) = next {
            self.run_move_command(
                &photon_core::movement::Movement::Offset(start),
                None,
                mods,
            );
        }
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            photon_core::cursor::CursorMode::Normal(offset) => {
                photon_core::selection::SelRegion::caret(*offset)
            }
            photon_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            } => photon_core::selection::SelRegion::new(
                *start.min(end),
                doc.buffer.with_untracked(|buffer| {
            ...
            photon_core::cursor::CursorMode::Insert(selection) => {
                        ...
                        && pointer_event.modifiers.control())
                {
                    ...
                    match rs {
                        ...
                        | FindHintRs::NoMatchContinue { .. } => {
                            self.common.photon_command.send(PhotonCommand {
    ...
    fn right_click(&self, pointer_event: &PointerInputEvent) {
                ...
                .unwrap_or_default()
            {
                vec![
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::RevealInPanel,
                    )),
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::RevealInFileExplorer,
                    )),
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::SourceControlOpenActiveFileRemoteUrl,
                    )),
                    None,
                    ...
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::AddRunDebugConfig,
                    )),
                    None,
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::PaletteCommand,
                    )),
                ]
            } else {
                vec![
                    ...
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::ShowCallHierarchy,
                    )),
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::FindReferences,
                    )),
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::GoToImplementation,
                    )),
                    Some(CommandKind::Focus(FocusCommand::Rename)),
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::RunInTerminal,
                    )),
                    None,
                    Some(CommandKind::Workbench(
                        PhotonWorkbenchCommand::RevealInPanel,
        ...
        if self.diff_editor_id.get_untracked().is_some() && is_file {
            cmds.push(Some(CommandKind::Workbench(
                PhotonWorkbenchCommand::GoToLocation,
            )));
        }
        let photon_command = self.common.photon_command;
        for cmd in cmds {
            if let Some(cmd) = cmd {
                menu = menu.entry(
...
impl KeyPressFocus for EditorData {
    ...
    fn run_command(
        &self,
        command: &crate::command::PhotonCommand,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
        if self.common.find.visual.get_untracked() && self.find_focus.get_untracked()
        ...
        match &command.kind {
            crate::command::CommandKind::Workbench(cmd) => {
                if let Some(digit) = cmd.quick_jump_digit() {
                    // Alt+digit jumps above (up), Alt+Shift+digit jumps below (down).
                    // Alt+0 (or Alt+Shift+0) stays on the current line.
                    // All jumps land at the end of the target line.
                    let upward = !mods.shift();
                    return self.quick_jump_to_line_end(digit, upward);
                }
                CommandExecuted::No
            }
