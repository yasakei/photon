use floem::{
    ...
    prelude::SignalTrack,
    reactive::{Memo, SignalGet, SignalWith, create_effect},
...
pub fn editor_gutter_view(
    ...
    // direction with Shift), and keeps relative numbers in sync.
    {
        ...
        let shift_held = editor.common.window_common.shift_held;
...
impl View for EditorGutterView {
    ...
    fn paint(&mut self, cx: &mut floem::context::PaintCx) {
        ...
        let attrs = Attrs::new()
            .family(&family)
            .color(config.color(PhotonColor::EDITOR_DIM))
        ...
        // Alt quick-jump: while Alt is held (and enabled in settings), show
        // `0` for the current line and `1`-`9` in the jump direction only.
        // Alt alone jumps up (labels above), Alt+Shift jumps down (labels
        // below). All other visible lines keep their normal (absolute or
        // modal-relative) numbers so the gutter never goes blank.
        // Alt(+Shift)+0 stays on the current line, always to end of line.
        let show_quick_jump = kind_is_normal
            && config.editor.enable_alt_quick_jump
            && self
                .editor
                .common
                .window_common
                .alt_held
                .get_untracked();
        // Alt alone = up, Alt+Shift = down.
        let jump_downward = self
            .editor
            .common
            .window_common
            .shift_held
        ...
        // Helper for the normal (non-jump) label so quick-jump mode never
        // leaves a line blank. Mirrors the default/relative logic below.
        let normal_text = |line: usize| -> String {
            if show_relative {
        ...
        screen_lines.with_untracked(|screen_lines| {
            for (line, y) in screen_lines.iter_lines_y() {
                ...
                let (text, is_current) = if show_quick_jump {
                    let rel = line as i64 - current_line as i64;
                    if rel == 0 {
                        ("0".to_string(), true)
                    } else {
                        let in_dir = if jump_downward {
                            rel > 0
                        } else {
                            rel < 0
                        };
                        let dist = rel.abs() as usize;
                        if in_dir && (1..=9).contains(&dist) {
                            (dist.to_string(), false)
                        } else {
                            (normal_text(line), false)
                        }
                    }
                } else if show_relative {
                    let num = if line == current_line {
                        line + 1
                    } else {
                        line.abs_diff(current_line)
                    };
                    (num.to_string(), line == current_line)
                } else {
                    ((line + 1).to_string(), line == current_line)
                };
