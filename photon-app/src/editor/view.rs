...
use crate::{
    ...
    config::{PhotonConfig, color::PhotonColor, editor::WrapStyle, icon::PhotonIcons},
    debug::{DapData, PhotonBreakpoint},
    doc::DocContent,
...
fn editor_wrap(config: &PhotonConfig) -> WrapMethod {
...
pub fn editor_style(
    ...
    .set(CursorColor, config.color(PhotonColor::EDITOR_CARET))
    .set(SelectionColor, config.color(PhotonColor::EDITOR_SELECTION))
    .set(
        CurrentLineColor,
        config.color(PhotonColor::EDITOR_CURRENT_LINE),
    )
    .set(
        VisibleWhitespaceColor,
        config.color(PhotonColor::EDITOR_VISIBLE_WHITESPACE),
    )
    .set(
        IndentGuideColor,
        config.color(PhotonColor::EDITOR_INDENT_GUIDE),
    )
    .set(ScrollBeyondLastLine, config.editor.scroll_beyond_last_line)
    .color(config.color(PhotonColor::EDITOR_FOREGROUND))
    .set(TextColor, config.color(PhotonColor::EDITOR_FOREGROUND))
    .set(PhantomColor, config.color(PhotonColor::EDITOR_DIM))
    .set(PlaceholderColor, config.color(PhotonColor::EDITOR_DIM))
    .set(
        PreeditUnderlineColor,
        config.color(PhotonColor::EDITOR_FOREGROUND),
    )
...
impl EditorView {
            ...
            match section.kind {
                ...
                DiffSectionKind::Added => {
                    cx.fill(
                        ...
                        config
                            .color(PhotonColor::SOURCE_CONTROL_ADDED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
                DiffSectionKind::Removed => {
                    cx.fill(
                        ...
                        config
                            .color(PhotonColor::SOURCE_CONTROL_REMOVED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
    ...
    fn paint_diff_no_code(
            ...
            .step_by(8)
        {
            ...
            if height > 0.0 {
                ...
                cx.stroke(
                    &Line::new(p0, p1),
                    config.color(PhotonColor::EDITOR_DIM),
                    &Stroke::new(1.0),
                );
            }
        }
    ...
    fn paint_current_line(
        ...
        if let Some(breakline) = breakline {
            if let Some(info) = screen_lines.info_for_line(breakline) {
                ...
                cx.fill(
                    &rect,
                    config.color(PhotonColor::EDITOR_DEBUG_BREAK_LINE),
                    0.0,
                );
            }
        }
    ...
    fn paint_find(&self, cx: &mut PaintCx, screen_lines: &ScreenLines) {
        ...
        let config = config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let color = config.color(PhotonColor::EDITOR_FOREGROUND);
    ...
    fn paint_sticky_headers(
        ...
        cx.fill(
            &sticky_area_rect,
            config.color(PhotonColor::EDITOR_STICKY_HEADER_BACKGROUND),
            0.0,
        );
    ...
    fn paint_scroll_bar(
        ...
        viewport: Rect,
        is_local: bool,
        config: Arc<PhotonConfig>,
    ) {
        // Honor the `scroll-width` UI setting instead of a hardcoded width
        // so the bar (and its change markers) can actually be resized.
        ...
        cx.fill(
            &Rect::ZERO
                ...
                .inflate(0.0, 10.0),
            config.color(PhotonColor::PHOTON_SCROLL_BAR),
            0.0,
        );
        ...
        // Markers fill most of the gutter so they stay visible even at
        // the default 10px width (previously a fixed 3px sliver).
        let marker_width = (bar_width - 4.0).max(2.0);
        let marker_x = viewport.x0 + total_width - bar_width + 2.0;
        for (y, height, _, color) in colors {
            let y = y / content_height * total_height;
            let height = ((height * line_height) as f64 / content_height
                * total_height)
            .max(3.0);
            let rect = Rect::ZERO.with_size(Size::new(marker_width, height)).with_origin(
                Point::new(
                    marker_x,
                    y + viewport.y0,
                ),
    ...
    fn paint_scope_lines(
        ...
        let config = self.editor.common.config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let brush = config.color(PhotonColor::EDITOR_FOREGROUND);
        ...
        stack((
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    // .border_bottom(1.0)
                    // .border_color(
                    //     config.get_color(PhotonColor::PHOTON_BORDER),
                    // )
                    .apply_if(
...
fn editor_gutter_breakpoint_view(
    ...
    breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, PhotonBreakpoint>>>,
    ...
    container(
        svg(move || config.get().ui_svg(PhotonIcons::DEBUG_BREAKPOINT)).style(
            move |s| {
                let config = config.get();
                let size = config.ui.icon_size() as f32 + 2.0;
                s.size(size, size)
                    .color(config.color(PhotonColor::DEBUG_BREAKPOINT_HOVER))
...
fn editor_gutter_breakpoints(
        ...
        stack((
            ...
            dyn_stack(
                ...
                move |(line, breakpoint)| {
                    ...
                    container(
                        svg(move || {
                            config.get().ui_svg(PhotonIcons::DEBUG_BREAKPOINT)
                        })
                        .style(move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32 + 2.0;
                            let color = if active {
                                PhotonColor::DEBUG_BREAKPOINT
                            } else {
                                PhotonColor::EDITOR_DIM
                            };
    ...
    .style(move |s| {
        s.absolute()
            ...
            .background(config.get().color(PhotonColor::EDITOR_BACKGROUND))
    })
}
...
fn editor_gutter_code_lens_view(
    ...
    let view = container(svg(move || config.get().ui_svg(PhotonIcons::START)).style(
        move |s| {
            let config = config.get();
    ...
    let view = container(
        svg(move || {
            let icon_str = match folding_display_item {
                FoldingDisplayItem::UnfoldStart(_) => PhotonIcons::FOLD_DOWN,
                FoldingDisplayItem::Folded(_) => PhotonIcons::FOLD,
                FoldingDisplayItem::UnfoldEnd(_) => PhotonIcons::FOLD_UP,
            };
...
fn editor_gutter_code_actions(
    ...
    container(
        container(
            svg(move || config.get().ui_svg(PhotonIcons::LIGHTBULB)).style(
                move |s| {
                    let config = config.get();
                    let size = config.ui.icon_size() as f32;
                    s.size(size, size)
                        .color(config.color(PhotonColor::PHOTON_WARN))
                },
            ),
        )
        .on_click_stop(move |_| {
            ...
            s.padding(4.0)
                ...
                .hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.color(PhotonColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
                .active(|s| {
                    s.background(
                        ...
                        move |(i, section)| {
                            stack((
                                svg(move || {
                                    config
                                        .get()
                                        .ui_svg(PhotonIcons::BREADCRUMB_SEPARATOR)
                                })
                                .style(move |s| {
                                    let config = config.get();
                                    let size = config.ui.icon_size() as f32;
...
fn search_editor_view(
    ...
    stack((
        ...
        clickable_icon(
            || PhotonIcons::SEARCH_CASE_SENSITIVE,
            move || {
    ...
    container(
        stack((
                ...
                clickable_icon(
                    move || {
                        if replace_active.get() {
                            PhotonIcons::ITEM_OPENED
                        } else {
                            PhotonIcons::ITEM_CLOSED
                        }
                    },
                    move || {
            ...
            stack((
                ...
                .style(|s| s.padding_left(6.0)),
                clickable_icon(
                    || PhotonIcons::SEARCH_REPLACE_ALL,
                    move || {
                        let text = replace_doc
                            .get_untracked()
                            .buffer
    ...
    changes.iter().map(move |change| {
        ...
        let color = match change {
            DiffLines::Left(_range) => {
                Some(config.color(PhotonColor::SOURCE_CONTROL_REMOVED))
            }
            DiffLines::Right(_range) => {
                ...
                if modified {
                    Some(config.color(PhotonColor::SOURCE_CONTROL_MODIFIED))
                } else {
                    Some(config.color(PhotonColor::SOURCE_CONTROL_ADDED))
                }
            }
            _ => None,
        };
