...
fn get_github_api(url: &str) -> Result<String> {
    let user_agent = format!("Photon/{}", photon_core::meta::VERSION);
    let resp = photon_proxy::get_url(url, Some(user_agent.as_str()))?;
...
pub fn find_grammar_release() -> Result<ReleaseInfo> {
    let releases: Vec<ReleaseInfo> = serde_json::from_str(&get_github_api(
        // Photon has no grammar releases of its own yet, use upstream Lapce grammars.
    ...
    use photon_core::meta::{RELEASE, ReleaseType, VERSION};
