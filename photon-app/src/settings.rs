...
/// Title suffix for settings that Photon added on top of Lapce upstream,
/// so users can tell them apart. `kind` is the capitalized section name
/// (e.g. `"Editor"`) and `field` is the kebab-case setting key.
fn photon_setting_suffix(kind: &str, field: &str) -> &'static str {
    match (kind, field) {
...
impl KeyPressFocus for SettingsData {
    fn get_mode(&self) -> photon_core::mode::Mode {
        Mode::Insert
    }
    ...
    fn run_command(
        &self,
        _command: &crate::command::PhotonCommand,
        _count: Option<usize>,
        _mods: Modifiers,
            ...
            for (kind, fields, descs, mut settings_map) in [
                ...
                for (name, desc) in fields.iter().zip(descs.iter()) {
                    ...
                    let name = format!(
                        ...
                        photon_setting_suffix(kind, &field)
                    );
        ...
        .style(move |s| {
            ...
            s.padding_horiz(20.0)
                ...
                .apply_if(kind == current_kind.get(), |s| {
                    s.background(config.color(PhotonColor::PANEL_CURRENT_BACKGROUND))
                })
                .hover(|s| {
                    ...
                    let token =
                        exec_after(Duration::from_millis(500), move |token| {
                            ...
                            if let Some(value) = value {
                                PhotonConfig::update_file(&kind, &field, value);
                            }
                        });
                    timer.set(token);
                        ...
                        text("Reset")
                            ...
                            .style(move |s| {
                                ...
                                s.margin_left(10)
                                    ...
                                    .border_radius(6)
                                    .border_color(
                                        config.color(PhotonColor::PHOTON_BORDER),
                                    )
...
fn dropdown_view(
    ...
    stack((
        ...
        container(
            svg(move || {
                if expanded.get() {
                    config.get().ui_svg(PhotonIcons::CLOSE)
                } else {
                    config.get().ui_svg(PhotonIcons::DROPDOWN_ARROW)
                }
            })
            .style(move |s| {
                let config = config.get();
        ...
        label(move || local_item_string.clone())
            ...
            .style(move |s| {
                s.text_ellipsis().padding_horiz(10.0).hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.get().color(PhotonColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
            })
    };
