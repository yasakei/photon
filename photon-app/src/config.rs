...
use photon_core::directory::Directory;
use photon_proxy::plugin::wasi::find_all_volts;
...
use crate::workspace::{PhotonWorkspace, PhotonWorkspaceType};
...
/// Surface opacity applied to theme background colors when
/// `core.window-transparent` is on (frosted glass over compositor blur).
const FROSTED_SURFACE_ALPHA: f32 = 0.84;

static DEFAULT_CONFIG: Lazy<config::Config> = Lazy::new(PhotonConfig::default_config);
static DEFAULT_PHOTON_CONFIG: Lazy<PhotonConfig> =
    Lazy::new(PhotonConfig::default_photon_config);
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        PhotonConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl PhotonConfig {
    pub fn load(
        ...
        let mut photon_config: PhotonConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
                error!("Failed to deserialize configuration file: {error}");
                DEFAULT_PHOTON_CONFIG.clone()
            }
        };

        photon_config.available_color_themes =
            Self::load_color_themes(disabled_volts, extra_plugin_paths);
        photon_config.available_icon_themes =
            Self::load_icon_themes(disabled_volts, extra_plugin_paths);
        photon_config.resolve_theme(workspace);

        photon_config.color_theme_list = photon_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        photon_config.color_theme_list.sort();

        photon_config.icon_theme_list = photon_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        photon_config.icon_theme_list.sort();

        photon_config.wrap_style_list = im::vector![
        ...
        photon_config.terminal.get_indexed_colors();
        ...
        match workspace.kind {
            PhotonWorkspaceType::Local => {
                if let Some(path) = workspace.path.as_ref() {
                    let path = path.join("./.photon/settings.toml");
                    config = config::Config::builder()
                        .add_source(config.clone())
                        .add_source(
            ...
            PhotonWorkspaceType::RemoteSSH(_) => {}
            #[cfg(windows)]
            PhotonWorkspaceType::RemoteWSL(_) => {}
        }
    ...
    fn default_photon_config() -> PhotonConfig {
        let mut default_photon_config: PhotonConfig =
        ...
        default_photon_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_photon_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_photon_config.resolve_colors(None);
        default_photon_config
    }

    fn resolve_theme(&mut self, workspace: &PhotonWorkspace) {
        let default_photon_config = DEFAULT_PHOTON_CONFIG.clone();
        ...
        self.resolve_colors(Some(&default_photon_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &PhotonWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &PhotonWorkspace, theme: &str) {
        self.core.icon_theme = theme.to_string();
        self.resolve_theme(workspace);
    }

    pub fn set_modal(&mut self, _workspace: &PhotonWorkspace, modal: bool) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&PhotonConfig>) {
        ...
        // Frosted glass: when the window is transparent, make every
        // background surface translucent so the compositor blur shows
        // through. Foregrounds/text stay fully opaque for readability.
        // Centralized here so it applies to any theme, built-in or custom.
        if self.core.window_transparent {
            for (key, color) in self.color.ui.iter_mut() {
                let is_surface = key.ends_with(".background")
                    || matches!(
                        key.as_str(),
                        "editor.current_line"
                            | "editor.selection"
                ...
                if is_surface {
                    let alpha = color.to_rgba8().a as f32 / 255.0
                        * FROSTED_SURFACE_ALPHA;
                    *color = color.with_alpha(alpha);
                }
            }
        }

        let fg = self.color(PhotonColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(PhotonColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(PhotonColor::PHOTON_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
    ...
    pub fn symbol_svg(&self, kind: &SymbolKind) -> Option<String> {
        let kind_str = match *kind {
            SymbolKind::ARRAY => PhotonIcons::SYMBOL_KIND_ARRAY,
            SymbolKind::BOOLEAN => PhotonIcons::SYMBOL_KIND_BOOLEAN,
            SymbolKind::CLASS => PhotonIcons::SYMBOL_KIND_CLASS,
            SymbolKind::CONSTANT => PhotonIcons::SYMBOL_KIND_CONSTANT,
            SymbolKind::ENUM_MEMBER => PhotonIcons::SYMBOL_KIND_ENUM_MEMBER,
            SymbolKind::ENUM => PhotonIcons::SYMBOL_KIND_ENUM,
            SymbolKind::EVENT => PhotonIcons::SYMBOL_KIND_EVENT,
            SymbolKind::FIELD => PhotonIcons::SYMBOL_KIND_FIELD,
            SymbolKind::FILE => PhotonIcons::SYMBOL_KIND_FILE,
            SymbolKind::INTERFACE => PhotonIcons::SYMBOL_KIND_INTERFACE,
            SymbolKind::KEY => PhotonIcons::SYMBOL_KIND_KEY,
            SymbolKind::FUNCTION => PhotonIcons::SYMBOL_KIND_FUNCTION,
            SymbolKind::METHOD => PhotonIcons::SYMBOL_KIND_METHOD,
            SymbolKind::OBJECT => PhotonIcons::SYMBOL_KIND_OBJECT,
            SymbolKind::NAMESPACE => PhotonIcons::SYMBOL_KIND_NAMESPACE,
            SymbolKind::NUMBER => PhotonIcons::SYMBOL_KIND_NUMBER,
            SymbolKind::OPERATOR => PhotonIcons::SYMBOL_KIND_OPERATOR,
            SymbolKind::TYPE_PARAMETER => PhotonIcons::SYMBOL_KIND_TYPE_PARAMETER,
            SymbolKind::PROPERTY => PhotonIcons::SYMBOL_KIND_PROPERTY,
            SymbolKind::STRING => PhotonIcons::SYMBOL_KIND_STRING,
            SymbolKind::STRUCT => PhotonIcons::SYMBOL_KIND_STRUCT,
            SymbolKind::VARIABLE => PhotonIcons::SYMBOL_KIND_VARIABLE,
            _ => return None,
        };
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (PhotonColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (PhotonColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (PhotonColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (PhotonColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (PhotonColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (PhotonColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (PhotonColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (PhotonColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (PhotonColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (PhotonColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (PhotonColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (PhotonColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (PhotonColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (PhotonColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (PhotonColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (PhotonColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (PhotonColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (PhotonColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (PhotonColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (PhotonColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (PhotonColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (PhotonColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (PhotonColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (PhotonColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (PhotonColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (PhotonColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (PhotonColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (PhotonColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (PhotonColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
