...
use photon_core::{command::FocusCommand, meta::VERSION, mode::Mode};

use crate::{
    command::{CommandExecuted, CommandKind},
    config::color::PhotonColor,
...
impl AboutUri {
    const PHOTON: &'static str = "https://lapce.dev";
        ...
        stack((
            svg(move || (config.get()).logo_svg()).style(move |s| {
                s.size(logo_size, logo_size)
                    .color(config.get().color(PhotonColor::EDITOR_FOREGROUND))
            }),
            label(|| "Photon".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
                    .color(config.get().color(PhotonColor::EDITOR_FOREGROUND))
            }),
            ...
            web_link(
                || "Website".to_string(),
                || AboutUri::PHOTON.to_string(),
                move || config.get().color(PhotonColor::EDITOR_LINK),
                internal_command,
            )
...
fn exclusive_popup<V: View + 'static>(
    ...
    .style(move |s| {
        ...
        .background(
            config
                .get()
                .color(PhotonColor::PHOTON_DROPDOWN_SHADOW)
                .multiply_alpha(0.5),
        )
    })
}
