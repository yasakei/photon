...
use photon_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::PhotonLanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use photon_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, PhotonCommand, WindowCommand,
    },
    db::PhotonDb,
    ...
    workspace::{PhotonWorkspace, PhotonWorkspaceType, SshHost},
};
...
impl PaletteData {
        ...
        let items = workspaces
            ...
            .filter_map(|w| {
                ...
                let filter_text = match &w.kind {
                    PhotonWorkspaceType::Local => text,
    ...
    fn get_ssh_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let PhotonWorkspaceType::RemoteSSH(host) = &workspace.kind {
    ...
    fn get_wsl_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let PhotonWorkspaceType::RemoteWSL(host) = &workspace.kind {
    ...
    fn set_run_configs(&self, content: String) {
        ...
        if configs.is_none() {
            if let Some(path) = self.workspace.path.as_ref() {
                let path = path.join(".photon").join("run.toml");
                self.common
                    .internal_command
    ...
    fn get_languages(&self) {
        let langs = PhotonLanguage::languages();
        let items = langs
            .iter()
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    ...
                    self.common.photon_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.photon_command.send(cmd.clone());
                }
                    ...
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: PhotonWorkspace {
                                kind: PhotonWorkspaceType::RemoteSSH(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::WslHost { host } => {
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: PhotonWorkspace {
                                kind: PhotonWorkspaceType::RemoteWSL(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match PhotonLanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .photon_command
                        .send(crate::command::PhotonCommand {
                        kind: CommandKind::Workbench(
                            crate::command::PhotonWorkbenchCommand::CheckoutReference,
                        ),
            ...
            self.common.window_common.window_command.send(
                WindowCommand::SetWorkspace {
                    workspace: PhotonWorkspace {
                        kind: PhotonWorkspaceType::RemoteSSH(ssh),
                        path: None,
                        last_open: 0,
                    },
                },
