...
pub fn status(
    ...
    _config: ReadSignal<Arc<PhotonConfig>>,
) -> impl View {
        ...
        stack((
            ...
            .style(move |s| {
                ...
                let (bg, fg) = match mode.get() {
                    Mode::Normal => (
                        PhotonColor::STATUS_MODAL_NORMAL_BACKGROUND,
                        PhotonColor::STATUS_MODAL_NORMAL_FOREGROUND,
                    ),
                    Mode::Insert => (
                        PhotonColor::STATUS_MODAL_INSERT_BACKGROUND,
                        PhotonColor::STATUS_MODAL_INSERT_FOREGROUND,
                    ),
                    Mode::Visual(_) => (
                        PhotonColor::STATUS_MODAL_VISUAL_BACKGROUND,
                        PhotonColor::STATUS_MODAL_VISUAL_FOREGROUND,
                    ),
                    Mode::Terminal => (
                        PhotonColor::STATUS_MODAL_TERMINAL_BACKGROUND,
                        PhotonColor::STATUS_MODAL_TERMINAL_FOREGROUND,
                    ),
                };
            ...
            stack((
                svg(move || config.get().ui_svg(PhotonIcons::SCM)).style(move |s| {
                    let config = config.get();
                ...
                label(branch).style(move |s| {
                    s.margin_left(10.0)
                        .color(config.get().color(PhotonColor::STATUS_FOREGROUND))
                        .selectable(false)
                }),
            ))
            .style(move |s| {
            ...
            .on_event(
                ...
                move |_| {
                    if pointer_down.get() {
                        workbench_command
                            .send(PhotonWorkbenchCommand::PaletteSCMReferences);
                    }
            ...
            ),
            {
                ...
                stack((
                    svg(move || config.get().ui_svg(PhotonIcons::ERROR)).style(
                        move |s| {
                            let config = config.get();
                        ...
                        move |s| {
                            s.margin_left(5.0)
                                .color(
                                    config
                                        .get()
                                        .color(PhotonColor::STATUS_FOREGROUND),
                                )
                                .selectable(false)
                        },
                    ),
                    svg(move || config.get().ui_svg(PhotonIcons::WARNING)).style(
                        move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32;
                            s.size(size, size)
        ...
        stack((
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            PhotonIcons::SIDEBAR_LEFT
                        } else {
                            PhotonIcons::SIDEBAR_LEFT_OFF
                        }
                    }
                };
                clickable_icon(
                ...
                let icon = {
                    ...
                    move || {
                        if panel.is_container_shown(
                            ...
                            PhotonIcons::LAYOUT_PANEL
                        } else {
                            PhotonIcons::LAYOUT_PANEL_OFF
                        }
                    }
                };
                clickable_icon(
            ...
            },
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            PhotonIcons::SIDEBAR_RIGHT
                        } else {
                            PhotonIcons::SIDEBAR_RIGHT_OFF
                        }
                    }
                };
                clickable_icon(
    ...
    .style(move |s| {
        ...
        s.border_top(1.0)
            ...
            .background(config.color(PhotonColor::STATUS_BACKGROUND))
