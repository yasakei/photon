...
mod tests {
    ...
    use crate::{config::PhotonConfig, workspace::PhotonWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = PhotonWorkspace::default();

        let config = PhotonConfig::merge_config(&workspace, None, None);
        let mut photon_config: PhotonConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        photon_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // photon_config.available_icon_themes = Some(vec![]);
        photon_config.core.color_theme = "test".to_string();
        ...
        // test basic override
        assert_eq!(
            photon_config.color("photon.error"),
            Color::WHITE,
            "Failed to get basic theme override"
        );
        // test that it falls through to the dark theme for unspecified color
        assert_eq!(
            photon_config.color("photon.warn"),
            Color::from_rgb8(0xE0, 0xAF, 0x68),
            "Failed to get from fallback dark theme"
        ); // $yellow (TokyoNight)
        // test that our custom variable worked; editor.background is a
        // surface, so the frosted-glass alpha applies (0.84)
        assert_eq!(
            photon_config.color("editor.background"),
            Color::from_rgb8(0xFF, 0x00, 0xFF).with_alpha(0.84),
            "Failed to get from custom variable"
        );
        // test that for text it now uses our redeclared variable
        assert_eq!(
            photon_config.color("editor.foreground"),
            Color::BLACK,
