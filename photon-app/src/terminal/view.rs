...
use photon_rpc::{proxy::ProxyRpcHandler, terminal::TermId};
...
impl TerminalView {
    ...
    fn paint_content(
        ...
        let term_bg = config.color(PhotonColor::TERMINAL_BACKGROUND);
        ...
        if let Some((c, x)) = line_content.cursor {
            ...
            let cursor_color = if mode == Mode::Terminal {
                if self.run_config.with_untracked(|run_config| {
                    ...
                    config.color(PhotonColor::TERMINAL_CURSOR)
                }
            } else {
                config.color(PhotonColor::EDITOR_CARET)
            };
            if self.is_focused {
...
impl View for TerminalView {
    ...
    fn compute_layout(
        ...
        if size != self.size {
            ...
            // Resizing the pty reallocates the grid and sends an RPC to the
            // proxy. During a panel/split drag the pixel size changes every
            // frame while the cell count only changes every ~8px, so skip the
            // expensive resize unless the cell dimensions actually changed.
            let (cur_width, cur_height) = {
                let raw = self.raw.read();
                (raw.term.columns(), raw.term.screen_lines())
            };
    ...
    fn paint(&mut self, cx: &mut floem::context::PaintCx) {
        ...
        if let Some(error) = self.launch_error.get() {
            ...
            text_layout.set_text(
                ...
                AttrsList::new(
                    attrs.color(config.color(PhotonColor::EDITOR_FOREGROUND)),
                ),
                None,
            );
            cx.draw_text(
        ...
        if let Some(selection) = content.selection.as_ref() {
            ...
            for line in start_line..end_line + 1 {
                ...
                cx.fill(
                    ...
                    config.color(PhotonColor::EDITOR_SELECTION),
                    0.0,
                );
            }
        ...
        //                 cx.stroke(
        //                     &rect,
        //                     config.get_color(PhotonColor::TERMINAL_FOREGROUND),
        //                     1.0,
        //                 );
        //                 start = *m.end();
