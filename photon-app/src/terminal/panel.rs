...
impl TerminalPanelData {
    ...
    /// Append a plain shell terminal to an existing tab, launched in
    /// `workdir` (workspace root when `None`). Used by session restore.
    pub fn new_terminal_in_tab(
        &self,
        tab: &TerminalTabData,
        ...
        let terminal_data = TerminalData::new(
            ...
            workdir_profile(workdir),
            self.common.clone(),
        );
        let i = terminal_data.scope.create_rw_signal(0);
        tab.terminals.update(|terminals| {
            terminals.push_back((i, terminal_data));
        });
    }

    /// Rebuild terminal tabs from a saved session, replacing the default
    /// single terminal. Plain shells reopen with their saved launch
    /// workdirs; run/debug terminals are never persisted.
    pub fn restore_terminal_tabs(
        &self,
        tabs: &[crate::workspace::TerminalTabState],
        active_tab: usize,
    ) {
        if tabs.is_empty() {
            return;
        }
        let ids: Vec<TerminalTabId> = self.tab_info.with_untracked(|info| {
            info.tabs.iter().map(|(_, tab)| tab.terminal_tab_id).collect()
        });
        for id in ids {
            self.close_tab(Some(id));
        }
        for state in tabs {
            if state.workdirs.is_empty() {
                continue;
            }
            let mut tab_data = None;
            for workdir in &state.workdirs {
                match tab_data.as_ref() {
                    None => {
                        tab_data = Some(
                            self.new_tab_run_debug(
                                None,
                                workdir_profile(workdir.clone()),
                            ),
                        );
                    }
                    Some(tab) => {
                        self.new_terminal_in_tab(tab, workdir.clone());
                    }
                }
            }
            if let Some(tab) = tab_data {
                let len = tab.terminals.with_untracked(|t| t.len());
                tab.active.set(state.active.min(len.saturating_sub(1)));
            }
        }
        self.tab_info.update(|info| {
            info.active = active_tab.min(info.tabs.len().saturating_sub(1));
        });
    ...
    fn get_run_config_by_name(&self, name: &str) -> Option<RunDebugConfig> {
        if let Some(workspace) = self.common.workspace.path.as_deref() {
            let run_toml = workspace.join(".photon").join("run.toml");
...
/// Build a terminal profile that launches a plain shell in `workdir`.
/// Returns `None` when there is no workdir to restore, so the caller
/// falls back to the default profile (workspace root).
fn workdir_profile(workdir: Option<PathBuf>) -> Option<TerminalProfile> {
    let workdir = workdir?;
    let url = url::Url::from_file_path(&workdir).ok()?;
    let mut profile = TerminalProfile::default();
    profile.workdir = Some(url);
    Some(profile)
}
