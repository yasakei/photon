...
pub struct TerminalData {
    ...
    /// Directory the shell was launched in (resolved profile workdir).
    /// Persisted per workspace so terminals reopen where they were.
    pub workdir: RwSignal<Option<PathBuf>>,
...
impl TerminalData {
    ...
    pub fn new_run_debug(
        ...
        let (raw, workdir) = Self::new_raw_terminal(
            &workspace,
            term_id,
            run_debug.as_ref(),
            profile,
        ...
        Self {
            ...
            common,
            launch_error,
            workdir: cx.create_rw_signal(workdir),
        }
    }
    ...
    ) -> (Arc<RwLock<RawTerminal>>, Option<PathBuf>) {
...
impl ExpandedRunDebug {
    pub fn expand(
        ...
        let mut program = if program == "${photon}" {
            std::env::current_exe()
                .map_err(|e| {
                    anyhow!(
                        "Failed to get current exe for ${{photon}} run and debug: {e}"
                    )
                })?
                .to_str()
                .ok_or_else(|| anyhow!("Failed to convert ${{photon}} path to str"))?
                .to_string()
        } else {
            program
        };
