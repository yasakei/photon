...
pub fn start_remote(
    ...
    let remote_proxy_path = match platform {
        Windows => format!(
            "%HOMEDRIVE%%HOMEPATH%\\AppData\\Local\\photon\\{}\\data\\proxy",
            meta::NAME
        ),
        Darwin => format!(
            "~/Library/Application\\ Support/dev.photon.{}/proxy",
            meta::NAME
        ),
        _ => {
    ...
    let remote_proxy_file = match platform {
        Windows => format!("{remote_proxy_path}\\photon.exe"),
        _ => format!("{remote_proxy_path}/photon"),
    };
    ...
    if !remote
        ...
        .map(|output| {
            if meta::RELEASE == ReleaseType::Debug {
                String::from_utf8_lossy(&output.stdout).starts_with("Photon-proxy")
            } else {
                String::from_utf8_lossy(&output.stdout).trim()
                    == format!("Photon-proxy {}", meta::VERSION)
            }
        })
        .unwrap_or(false)
    {
...
fn download_remote(
    ...
    let script_install = match platform {
        HostPlatform::Windows => {
            ...
            let remote_proxy_script = "${env:TEMP}\\photon-proxy.ps1";
    ...
    if !script_install.success() {
        ...
        if !cmd.success() {
            let proxy_filename = format!("photon-proxy-{platform}-{architecture}");
            ...
            let mut resp = photon_proxy::get_url(url, None).expect("request failed");
