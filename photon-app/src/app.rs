...
use crate::{
    ...
    command::{
        CommandKind, InternalCommand, PhotonCommand, PhotonWorkbenchCommand,
        WindowCommand,
    },
    config::{
        PhotonConfig, color::PhotonColor, icon::PhotonIcons, ui::TabSeparatorHeight,
    ...
    workspace::{PhotonWorkspace, PhotonWorkspaceType},
};
...
#[derive(Debug)]
struct Cli {
    /// Launch new window even if Photon is already running
    ...
    #[clap(value_parser = photon_proxy::cli::parse_file_line_column)]
...
impl AppData {
    pub fn reload_config(&self) {
        let config =
            PhotonConfig::load(&PhotonWorkspace::default(), &[], &self.plugin_paths);
    ...
    fn default_window_config(&self) -> WindowConfig {
        // Frosted glass: transparent window + translucent theme surfaces
        // (see resolve_colors) lets the compositor blur the wallpaper
        // behind the app. Needs a compositor blur rule; restart required.
        let transparent =
            self.config.get_untracked().core.window_transparent;
    ...
    fn create_windows(
        ...
        if !dirs.is_empty() {
            ...
            for dir in dirs {
                ...
                {
                    PhotonWorkspaceType::RemoteWSL(crate::workspace::WslHost {
                        host: String::new(),
                    })
                } else {
                    PhotonWorkspaceType::Local
                };
                #[cfg(not(windows))]
                let workspace_type = PhotonWorkspaceType::Local;
                ...
                let info = WindowInfo {
                    ...
                    tabs: TabsInfo {
                        active_tab: 0,
                        workspaces: vec![PhotonWorkspace {
                            kind: workspace_type,
        ...
        if initial_windows == 0 {
            let mut info = db.get_window().unwrap_or_else(|_| WindowInfo {
                ...
                tabs: TabsInfo {
                    active_tab: 0,
                    workspaces: vec![PhotonWorkspace::default()],
                },
            });
            info.tabs = TabsInfo {
                active_tab: 0,
    ...
    fn app_view(
        ...
        let key_up_window_data = key_down_window_data.clone();
        let focus_lost_window_data = key_down_window_data.clone();
        let pointer_window_data = key_down_window_data.clone();
        let window_lost_focus_data = key_down_window_data.clone();
        view.window_scale(move || window_scale.get())
            .keyboard_navigable()
            .on_event(EventListener::KeyDown, move |event| {
                if let Event::KeyDown(key_event) = event {
                    key_down_window_data.update_alt_held_from_key_down(key_event);
            ...
            .on_event(EventListener::KeyUp, move |event| {
                if let Event::KeyUp(key_event) = event {
                    key_up_window_data.update_alt_held_from_key_up(key_event);
                    EventPropagation::Stop
                } else {
                    EventPropagation::Continue
                }
            ...
            .on_event(EventListener::FocusLost, move |_| {
                focus_lost_window_data.clear_alt_held();
                EventPropagation::Continue
            })
            .on_event(EventListener::WindowLostFocus, move |_| {
                // Alt+Tab away while holding Alt misses the KeyUp, which left
                // the gutter stuck in Alt preview on return. Clear it.
                window_lost_focus_data.clear_alt_held();
                EventPropagation::Continue
            })
            .on_event(EventListener::PointerDown, {
                ...
                move |event| {
                    if let Event::PointerDown(pointer_event) = event {
                        // Sync (usually clears) the Alt preview on click so a
                        // stuck Alt state returns to normal line numbers.
                        // Preserves Alt+click (multi-cursor) via its modifiers.
                        pointer_window_data
                            .sync_modifiers_from_pointer(pointer_event.modifiers);
    ...
    let view_fn = move |(i, layout_rect, child): (
        ...
        let child_view = {
                            ...
                            .apply_if(
                                !info.with(|info| info.is_pristine)
                                    && config.ui.tab_close_button
                                        == TabCloseButton::Off,
                                |s| s.color(config.color(PhotonColor::PHOTON_WARN)),
                            )
                    })
            })
            .style(|s| s.padding(4.));
            ...
            .style(move |s| {
                s.items_center()
                    ...
                    .border_right(1.0)
                    .border_color(config.get().color(PhotonColor::PHOTON_BORDER))
                    .padding_horiz(6.)
                    .gap(6.)
                    .grid()
                ...
                .style(move |s| {
                    s.size_full()
                        ...
                        .border_color(config.get().color(if is_focused() {
                            PhotonColor::PHOTON_TAB_ACTIVE_UNDERLINE
                        } else {
                            PhotonColor::PHOTON_TAB_INACTIVE_UNDERLINE
                        }))
                })
                .style(|s| {
                    s.absolute()
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .border_color(
                            config
                                .get()
                                .color(PhotonColor::PHOTON_TAB_ACTIVE_UNDERLINE)
                                .multiply_alpha(0.5),
                        )
                })
        ...
        .style(move |s| {
            ...
            s.height_full()
                ...
                .hover(|s| s.background(config.color(PhotonColor::HOVER_BACKGROUND)))
        })
            ...
            (
                clip(empty().style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .box_shadow_blur(3.0)
                        .box_shadow_color(
                            config.color(PhotonColor::PHOTON_DROPDOWN_SHADOW),
                        )
                }))
                .style(move |s| {
                ...
                stack((
                    clickable_icon(
                        || PhotonIcons::TAB_PREVIOUS,
                        move || {
                            workbench_command
                                .send(PhotonWorkbenchCommand::PreviousEditorTab);
                        },
                        || false,
                        || false,
                        || "Previous Tab",
                    ...
                    clickable_icon(
                        || PhotonIcons::TAB_NEXT,
                        move || {
                            workbench_command
                                .send(PhotonWorkbenchCommand::NextEditorTab);
                        },
                        || false,
                        || false,
                        || "Next Tab",
                    ...
                    stack((
                        ...
                        .style(move |s| {
                            s.height_full()
                                ...
                                .border_right(1.0)
                                .border_color(
                                    config.get().color(PhotonColor::PHOTON_BORDER),
                                )
                        }),
                        container(
                            editor_container_view(
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(
                            config
                                .get()
                                .color(PhotonColor::EDITOR_DRAG_DROP_BACKGROUND),
                        )
                })
...
fn split_resize_border(
    ...
    dyn_stack(
        ...
        move |(index, (_, content))| {
            // Snapshot drag start in window coords plus initial child sizes.
            // `pointer_event.pos` is local to the 4px handle (which itself moves
            // as the split resizes), so a local delta stalls near zero. Using
            // window coords with the initial snapshot avoids both the stall and
            // the double-counting/overshoot from re-reading live rects each move.
            // Moves are delivered via `ResizeHandle::event_before_children`,
            // which keeps firing while active even when the cursor leaves the
            // thin handle (declarative `on_event(PointerMove)` is gated by
            // `rect.contains(pos)` and stops firing).
            const MIN_SIZE: f64 = 50.0;
            let drag_state: RwSignal<Option<(Point, Vec<f64>, f64, SplitDirection)>> =
                create_rw_signal(None);
            resize_handle()
                .on_down(move |window| {
                    let dir = direction(false);
                    let sizes = split.with_untracked(|split| {
                        split
                            .children
                            .iter()
                            .map(|(_, c)| {
                                let r = content_rect(c, false);
                                match dir {
                                    SplitDirection::Vertical => r.width(),
                                    SplitDirection::Horizontal => r.height(),
                                }
                            })
                            .collect::<Vec<f64>>()
                    });
                    ...
                    if total > 0.0 {
                        drag_state.set(Some((window, sizes, total, dir)));
                    }
                })
                .on_move(move |window| {
                    if let Some((start_window, initial_sizes, total, dir)) =
                        drag_state.get_untracked()
                    {
                        if total <= 0.0 {
                            return;
                        }
                        if index == 0 || index >= initial_sizes.len() {
                            return;
                        }
                        let delta = window - start_window;
                        let shift = match dir {
                            SplitDirection::Vertical => delta.x,
                            SplitDirection::Horizontal => delta.y,
                        };
                        let first_initial = initial_sizes[index - 1];
                        let second_initial = initial_sizes[index];
                        let min_shift = MIN_SIZE - first_initial;
                        let max_shift = second_initial - MIN_SIZE;
                        let shift = if min_shift <= max_shift {
                            shift.clamp(min_shift, max_shift)
                        } else {
                            return;
                        };
                        let first_new = first_initial + shift;
                        let second_new = second_initial - shift;
                        // Only touch the two adjacent children and skip no-op
                        // sets: notifying untouched siblings would schedule
                        // extra style/layout passes on every pointer move.
                        split.with_untracked(|split| {
                            if let Some((first_size, _)) =
                                split.children.get(index - 1)
                            {
                                let v = first_new / total;
                                if first_size.get_untracked() != v {
                                    first_size.set(v);
                                }
                            }
                            if let Some((second_size, _)) =
                                split.children.get(index)
                            {
                                let v = second_new / total;
                                if second_size.get_untracked() != v {
                                    second_size.set(v);
                                }
                            }
                        })
            ...
            .style(move |s| {
                ...
                let is_dragging = drag_state.get().is_some();
                ...
                s.position(Position::Absolute)
                    ...
                    .apply_if(is_dragging, |s| {
                        ...
                        .background(config.get().color(PhotonColor::EDITOR_CARET))
                    })
                    .hover(|s| {
                        s.cursor(match direction {
                            SplitDirection::Vertical => CursorStyle::ColResize,
    ...
    dyn_stack(
        ...
        move |(_, content)| {
            container(empty().style(move |s| {
                ...
                .background(config.get().color(PhotonColor::PHOTON_BORDER))
            }))
            .style(move |s| {
...
fn main_split(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.border_color(config.color(PhotonColor::PHOTON_BORDER))
            .background(config.color(PhotonColor::EDITOR_BACKGROUND))
    ...
    .style(move |s| {
        ...
        s.padding(4.0)
            ...
            .apply_if(active_fn(), |s| {
                s.border_color(config.color(PhotonColor::EDITOR_CARET))
            })
            .hover(|s| {
                s.cursor(CursorStyle::Pointer)
                    .background(config.color(PhotonColor::PANEL_HOVERED_BACKGROUND))
            })
            .active(|s| {
                s.background(
                    config.color(PhotonColor::PANEL_HOVERED_ACTIVE_BACKGROUND),
                )
            })
    });
...
fn tooltip_tip<V: View + 'static>(
    ...
    container(child).style(move |s| {
        ...
        s.padding_horiz(10.0)
            ...
            .color(config.color(PhotonColor::TOOLTIP_FOREGROUND))
            .background(config.color(PhotonColor::TOOLTIP_BACKGROUND))
            .border(1)
            .border_radius(6)
            .border_color(config.color(PhotonColor::PHOTON_BORDER))
            .box_shadow_blur(3.0)
            .box_shadow_color(config.color(PhotonColor::PHOTON_DROPDOWN_SHADOW))
            .margin_left(0.0)
            .margin_top(4.0)
    })
}
            ...
            container(
                stack((
                    ...
                    focus_text(
                        ...
                        move || config.get().color(PhotonColor::EDITOR_FOCUS),
                    )
                    ...
                    .style(move |s| {
                        s.color(config.get().color(PhotonColor::EDITOR_DIM))
                            .min_width(0.0)
                            .flex_grow(1.0f32)
                            .flex_basis(0.0)
                    }),
            ...
            container(
                stack((
                    svg(move || {
                        ...
                        config
                            .symbol_svg(&kind)
                            .unwrap_or_else(|| config.ui_svg(PhotonIcons::FILE))
                    })
                    .style(move |s| {
                        let config = config.get();
                        let size = config.ui.icon_size() as f32;
                        s.min_width(size).size(size, size).margin_right(5.0).color(
                            config.symbol_color(&kind).unwrap_or_else(|| {
                                config.color(PhotonColor::PHOTON_ICON_ACTIVE)
                            }),
                        )
                    }),
                    focus_text(
            ...
            container(
                stack((
                    svg(move || {
                        let config = config.get();
                        match mode {
                            RunDebugMode::Run => config.ui_svg(PhotonIcons::START),
                            RunDebugMode::Debug => config.ui_svg(PhotonIcons::DEBUG),
                        }
                    })
                    .style(move |s| {
                        let config = config.get();
    ...
    .style(move |s| {
        s.width_full()
            ...
            .apply_if(index.get() == i, |style| {
                style.background(
                    config.get().color(PhotonColor::PALETTE_CURRENT_BACKGROUND),
                )
            })
    })
}
                    ...
                    .style(move |s| {
                        s.width_full().cursor(CursorStyle::Pointer).hover(|s| {
                            s.background(
                                config
                                    .get()
                                    .color(PhotonColor::PANEL_HOVERED_BACKGROUND),
                            )
                        })
                    })
                },
    ...
    container(
        ...
        .style(move |s| {
            ...
            s.width(config.ui.palette_width() as f64)
                ...
                .border_color(config.color(PhotonColor::PHOTON_BORDER))
                .flex_col()
                .background(config.color(PhotonColor::PALETTE_BACKGROUND))
                .pointer_events_auto()
        }),
    )
    .style(move |s| {
            ...
            stack((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.get().ui_svg(PhotonIcons::ERROR)
                    } else {
                        config.get().ui_svg(PhotonIcons::WARNING)
                    }
                })
                .style(move |s| {
                    let config = config.get();
                    ...
                    let color = if let MessageType::ERROR = message.typ {
                        config.color(PhotonColor::PHOTON_ERROR)
                    } else {
                        config.color(PhotonColor::PHOTON_WARN)
                    };
                    s.min_width(size)
                        .size(size, size)
                        .margin_right(10.0)
                    ...
                    .style(move |s| {
                        ...
                        s.padding_horiz(5.0)
                            ...
                            .apply_if(active.get() == i, |s| {
                                s.background(
                                    config.color(PhotonColor::COMPLETION_CURRENT),
                                )
                            })
                            .hover(move |s| {
                                s.background(
...
fn code_action(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.get().color(PhotonColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
fn rename(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.get().color(PhotonColor::PANEL_BACKGROUND))
            .border_radius(6.0)
            .padding(6.0)
    })
    ...
    .style(move |s| {
        ...
        s.size_full()
            ...
            .class(floem::views::scroll::Handle, |s| {
                s.background(config.color(PhotonColor::PHOTON_SCROLL_BAR))
            })
    })
...
fn workspace_title(workspace: &PhotonWorkspace) -> Option<String> {
    let p = workspace.path.as_ref()?;
    let dir = p.file_name().unwrap_or(p.as_os_str()).to_string_lossy();
    Some(match &workspace.kind {
        PhotonWorkspaceType::Local => format!("{dir}"),
        PhotonWorkspaceType::RemoteSSH(remote) => format!("{dir} [{remote}]"),
        #[cfg(windows)]
        PhotonWorkspaceType::RemoteWSL(remote) => format!("{dir} [{remote}]"),
    })
}
        ...
        stack((
            ...
            .dragging_style(move |s| {
                ...
                s.border(1.0)
                    ...
                    .color(
                        config
                            .color(PhotonColor::EDITOR_FOREGROUND)
                            .multiply_alpha(0.7),
                    )
                    .background(
                        config
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    .height_full()
                    .border_color(
                        config.get().color(PhotonColor::PHOTON_TAB_ACTIVE_UNDERLINE),
                    )
        ...
        container(clickable_icon(
            ...
            move || {
                window_data.run_window_command(WindowCommand::NewWorkspaceTab {
                    workspace: PhotonWorkspace::default(),
                    end: true,
                });
            },
            || false,
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - Photon"),
            None => "Photon".to_string(),
        }
    })
    ...
    .window_menu(move || {
        ...
        if let Some((_, window_tab)) = window_tab {
            ...
            let photon_command = window_tab.common.photon_command;
            window_menu(photon_command, workbench_command)
        } else {
            Menu::new("Photon")
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up Photon..");
    ...
    #[cfg(feature = "vendored-fonts")]
    {
        ...
        const FONT_JETBRAINS_MONO_BOLD_ITALIC: &[u8] = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../extra/fonts/JetBrainsMono/JetBrainsMono-BoldItalic.ttf"
        ));
        ...
        FONT_SYSTEM
            .lock()
            .db_mut()
            .load_font_source(Source::Binary(Arc::new(FONT_JETBRAINS_MONO_BOLD)));
        FONT_SYSTEM
            .lock()
            .db_mut()
            .load_font_source(Source::Binary(Arc::new(
        ...
        FONT_SYSTEM
            ...
            .db_mut()
            .load_font_source(Source::Binary(Arc::new(
                FONT_JETBRAINS_MONO_BOLD_ITALIC,
            )));
    }
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .stderr(stderr)
            .stdout(stdout)
            .env("PHOTON_LOG", "photon_app::app=error,off")
            .spawn()
        {
            eprintln!("Failed to launch photon: {why}");
            std::process::exit(1);
        };
        return;
    }
    ...
    // in the existing Photon process
    if !cli.new {
        match get_socket() {
            Ok(socket) => {
    ...
    if let Err(err) = photon_proxy::register_photon_path() {
        tracing::error!("{:?}", err);
    }
    let db = match PhotonDb::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
            #[cfg(windows)]
            logging::error_modal("Error", &format!("Failed to create PhotonDb: {e}"));

            trace!(TraceLevel::ERROR, "Failed to create PhotonDb: {e}");
            std::process::exit(1);
        }
    };
    let scope = Scope::new();
    ...
    let mut watcher = notify::recommended_watcher(ConfigWatcher::new(tx))
        ...
        .unwrap();
    if let Some(path) = PhotonConfig::settings_file() {
    ...
    if let Some(path) = PhotonConfig::keymaps_file() {
        if let Err(err) = watcher.watch(&path, notify::RecursiveMode::Recursive) {
            tracing::error!("{:?}", err);
        }
    }
    ...
    let config = PhotonConfig::load(&PhotonWorkspace::default(), &[], &plugin_paths);
...
pub fn try_open_in_existing_process(
    ...
    photon_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    photon_rpc::stdio::read_msg(&mut reader)?;
...
pub fn window_menu(
    photon_command: Listener<PhotonCommand>,
    workbench_command: Listener<PhotonWorkbenchCommand>,
) -> Menu {
    Menu::new("Photon")
        .entry({
            let mut menu = Menu::new("Photon")
                .entry(MenuItem::new("About Photon").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::ShowAbout)
                }))
                .separator()
                .entry(
                    Menu::new("Settings...")
                        .entry(MenuItem::new("Open Settings").action(move || {
                            workbench_command
                                .send(PhotonWorkbenchCommand::OpenSettings);
                        }))
                        .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                            move || {
                                workbench_command.send(
                                    PhotonWorkbenchCommand::OpenKeyboardShortcuts,
                                );
                            },
                        )),
                )
                .separator()
                .entry(MenuItem::new("Quit Photon").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::Quit);
                }));
            if cfg!(target_os = "macos") {
                menu = menu
                    .separator()
                    .entry(MenuItem::new("Hide Photon"))
        ...
        .entry(
            Menu::new("File")
                .entry(MenuItem::new("New File").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::NewFile);
                }))
                .separator()
                .entry(MenuItem::new("Open").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::OpenFile);
                }))
                .entry(MenuItem::new("Open Folder").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::OpenFolder);
                }))
                .separator()
                .entry(MenuItem::new("Save").action(move || {
                    photon_command.send(PhotonCommand {
                        kind: CommandKind::Focus(FocusCommand::Save),
                        data: None,
                    });
                }))
                .entry(MenuItem::new("Save All").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::SaveAll);
                }))
                .separator()
                .entry(MenuItem::new("Close Folder").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::CloseFolder);
                }))
                .entry(MenuItem::new("Close Window").action(move || {
                    workbench_command.send(PhotonWorkbenchCommand::CloseWindow);
                })),
        )
        .entry(
            Menu::new("Edit")
