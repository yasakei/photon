...
use crate::command::PhotonWorkbenchCommand;
...
impl PaletteKind {
    ...
    /// Get the [`PhotonWorkbenchCommand`] that opens this palette kind, if one exists.
    pub fn command(self) -> Option<PhotonWorkbenchCommand> {
        match self {
            PaletteKind::PaletteHelp => Some(PhotonWorkbenchCommand::PaletteHelp),
            PaletteKind::Line => Some(PhotonWorkbenchCommand::PaletteLine),
            PaletteKind::DocumentSymbol => {
                Some(PhotonWorkbenchCommand::PaletteSymbol)
            }
            PaletteKind::WorkspaceSymbol => {
                Some(PhotonWorkbenchCommand::PaletteWorkspaceSymbol)
            }
            PaletteKind::Workspace => Some(PhotonWorkbenchCommand::PaletteWorkspace),
            PaletteKind::Command => Some(PhotonWorkbenchCommand::PaletteCommand),
            PaletteKind::File => Some(PhotonWorkbenchCommand::Palette),
            PaletteKind::HelpAndFile => {
                Some(PhotonWorkbenchCommand::PaletteHelpAndFile)
            }
            PaletteKind::Reference => None, // InternalCommand::PaletteReferences
            PaletteKind::SshHost => Some(PhotonWorkbenchCommand::ConnectSshHost),
            #[cfg(windows)]
            PaletteKind::WslHost => Some(PhotonWorkbenchCommand::ConnectWslHost),
            PaletteKind::RunAndDebug => {
                Some(PhotonWorkbenchCommand::PaletteRunAndDebug)
            }
            PaletteKind::ColorTheme => Some(PhotonWorkbenchCommand::ChangeColorTheme),
            PaletteKind::IconTheme => Some(PhotonWorkbenchCommand::ChangeIconTheme),
            PaletteKind::Language => Some(PhotonWorkbenchCommand::ChangeFileLanguage),
            PaletteKind::LineEnding => {
                Some(PhotonWorkbenchCommand::ChangeFileLineEnding)
            }
            PaletteKind::SCMReferences => {
                Some(PhotonWorkbenchCommand::PaletteSCMReferences)
            }
            PaletteKind::TerminalProfile => None, // InternalCommand::NewTerminal
            PaletteKind::DiffFiles => Some(PhotonWorkbenchCommand::DiffFiles),
        }
    }
