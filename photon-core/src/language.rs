...
use photon_rpc::style::{LineStyle, Style};
...
/// indices into the LANGUAGES array.  See method `PhotonLanguage::properties`.
///
...
/// `PhotonLanguage` as they will be accessed using the enum variants as indices.
const LANGUAGES: &[SyntaxProperties] = &[
    // Undetected/unmatched fallback or just plain file
    SyntaxProperties {
        id: PhotonLanguage::PlainText,
        indent: Indent::tab(),
        files: &[],
        extensions: &["txt"],
    ...
    SyntaxProperties {
        id: PhotonLanguage::Gitattributes,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
    ...
    SyntaxProperties {
        id: PhotonLanguage::KotlinBuildScript,
        indent: Indent::space(2),
        files: &[],
        extensions: &["kts"],
        comment: CommentProperties {
    ...
    SyntaxProperties {
        id: PhotonLanguage::MarkdownInline,
        indent: Indent::space(4),
    ...
    SyntaxProperties {
        id: PhotonLanguage::OcamlInterface,
        indent: Indent::space(2),
        files: &[],
        extensions: &["mli"],
        comment: comment_properties!("(*"),
    ...
    SyntaxProperties {
        id: PhotonLanguage::SshClientConfig,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
...
impl PhotonLanguage {
    ...
    pub fn from_path(path: &Path) -> PhotonLanguage {
        Self::from_path_raw(path).unwrap_or(PhotonLanguage::PlainText)
    }

    pub fn from_path_raw(path: &Path) -> Option<PhotonLanguage> {
    ...
    pub fn from_name(name: &str) -> Option<PhotonLanguage> {
        match PhotonLanguage::from_str(name.to_lowercase().as_str()) {
            Ok(v) => Some(v),
            Err(e) => {
                event!(Level::DEBUG, "failed parsing `{name}` PhotonLanguage: {e}");
                None
            }
        }
    }
    ...
    fn get_grammar(&self) -> Option<tree_sitter::Language> {
        ...
        if let Some(grammars_dir) = Directory::grammars_directory() {
            match self::load_grammar(&grammar_name, &grammar_fn_name, &grammars_dir)
            {
                ...
                Err(err) => {
                    if self != &PhotonLanguage::PlainText {
...
mod tests {
    ...
    use super::PhotonLanguage;

    #[test]
    fn test_language_from_path() {
        let l = PhotonLanguage::from_path(&PathBuf::new().join("test.rs"));
        assert_eq!(l, PhotonLanguage::Rust);
    }
}
