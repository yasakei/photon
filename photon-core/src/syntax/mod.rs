        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    PhotonLanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    PhotonLanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => PhotonLanguage::from_name(id),
            };
            language
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = PhotonLanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(PhotonLanguage::PlainText)
    }

    pub fn from_language(language: PhotonLanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == PhotonLanguage::Markdown {
