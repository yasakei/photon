...
#[test]
fn test_load_volt() {
    let photon_proxy_dir = std::env::current_dir()
        .expect("Can't get \"photon-proxy\" directory")
        .join("src")
        .join("plugin")
        .join("wasi")
        .join("plugins");
    ...
    let path = photon_proxy_dir.join("some-path");
    ...
    if let Ok(volt_metadata) = load_volt(&photon_proxy_dir) {
        panic!(
            "Unexpected result from `photon_proxy::plugin::wasi::load_volt` function: {volt_metadata:?}"
        );
    }

    // Invalid file (not readable into a string)
    // Making sure the file exists
    let path = photon_proxy_dir.join("smiley.png");
    ...
    let parent_path = photon_proxy_dir.join("some_author.test-plugin-one");
    ...
    assert_eq!(
        ...
        VoltMetadata {
            ...
            repository: Some("https://github.com/photon".to_string()),
            wasm: wasm_path,
    ...
    let parent_path = photon_proxy_dir.join("some_author.test-plugin-two");
    ...
    let parent_path = photon_proxy_dir.join("some_author.test-plugin-three");
