...
/// `plugin_dev_path` allows launching Photon with a plugin on your local system for testing
/// purposes.  
...
/// use std::fs::File;
/// use std::io::Write;
/// use photon_proxy::plugin::wasi::load_volt;
/// use photon_rpc::plugin::VoltMetadata;
///
...
pub fn start_volt(
    ...
    let stdout_photon = stdout.clone();
    linker.func_wrap("photon", "host_handle_rpc", move || {
        if let Ok(msg) = wasi_read_string(&stdout_photon) {
            if let Some(resp) =
    ...
    // Backward compat: volts built against the Lapce SDK import from the
    // `lapce` module. Keep them working alongside new `photon` volts.
    let local_rpc = rpc.clone();
    let local_stdin = stdin.clone();
    let volt_name = format!("volt {}", meta.name);
    let stdout_lapce = stdout.clone();
    linker.func_wrap("lapce", "host_handle_rpc", move || {
        if let Ok(msg) = wasi_read_string(&stdout_lapce) {
            if let Some(resp) =
                handle_plugin_server_message(&local_rpc, &msg, &volt_name)
            {
                if let Ok(msg) = serde_json::to_string(&resp) {
    ...
    let stderr_photon = stderr.clone();
    linker.func_wrap("photon", "host_handle_stderr", move || {
        if let Ok(msg) = wasi_read_string(&stderr_photon) {
            tracing_log::log::log!(target: &format!("photon_proxy::plugin::wasi::{}::{}", plugin_meta.author, plugin_meta.name), tracing_log::log::Level::Debug, "{msg}");
        }
    })?;
    let plugin_meta = meta.clone();
    let stderr_lapce = stderr.clone();
    linker.func_wrap("lapce", "host_handle_stderr", move || {
        if let Ok(msg) = wasi_read_string(&stderr_lapce) {
            tracing_log::log::log!(target: &format!("photon_proxy::plugin::wasi::{}::{}", plugin_meta.author, plugin_meta.name), tracing_log::log::Level::Debug, "{msg}");
        }
    })?;
